...
class IVDecisionTreeClassifier(_IVTreeBase, ClassifierMixin):
    ...
    def _impurity(self, y):
        ...
        if self.criterion == "gini":
            return 2.0 * p * (1.0 - p)
        # entropy / log_loss  (base-2 entropy, identical split quality)
    ...
    def _compute_gains_vectorized(self, cum, total, nl, nr, n, pi):
        """
        Vectorised gain for all (feature, bin) pairs.
        Supports:  gini · entropy · log_loss (≡ entropy)
        """
        sl = cum[:, :-1, 1]
        sr = total[:, 1:2] - sl
...
class IVDecisionTreeRegressor(_IVTreeBase, RegressorMixin):
    ...
    def _compute_gains_vectorized(self, cum, total, nl, nr, n, pi):
        """
        Vectorised gain for all (feature, bin) pairs.
        All histogram criteria use MSE/variance as the histogram-level split
        proxy (MAE/Huber require per-sample medians which can't be derived from
        cumulative bin sums).  The impurity function uses the real criterion.
        Friedman MSE uses a different gain formula: nl*nr/(nl+nr)*(mean_l-mean_r)².
        """
        ...
        if self.criterion == "friedman_mse":
            # Friedman (2001) split criterion: improvement = nl*nr/(nl+nr)*(ȳL-ȳR)²
            # Equivalent to weighted variance reduction, upweights balanced splits
            with np.errstate(divide="ignore", invalid="ignore"):
                mean_l = np.where(nl>0, sl/nl, 0.0)
                mean_r = np.where(nr>0, sr/nr, 0.0)
            weight = (nl * nr) / np.where(nl+nr>0, nl+nr, 1.0)
