...
def load_dash_app(app_path: str):
    ...
    # Strategie 1 : noms de variables courants, verification par MRO
    def _is_dash_or_flask(obj) -> bool:
        try:
            mro_names   = {cls.__name__ for cls in type(obj).__mro__}
            mro_modules = {cls.__module__ or "" for cls in type(obj).__mro__}
            if mro_names & {"Dash", "Flask", "DashBlueprint", "JupyterDash"}:
                return True
            if any("dash" in m or "flask" in m for m in mro_modules):
                return True
            if hasattr(obj, "server") and hasattr(obj, "layout"):
                return True
            if (hasattr(obj, "run") or hasattr(obj, "run_server")) and hasattr(obj, "callback"):
                return True
        except Exception:
            pass
        return False

    for candidate in ("app", "application", "server", "dash_app", "APP", "APPLICATION"):
        obj = getattr(module, candidate, None)
        if obj is not None and _is_dash_or_flask(obj):
            log.info(f"  ✓  App Dash trouvée → variable '{candidate}' ({type(obj).__name__})")
            return module, obj

    # Strategie 2 : scan de TOUTES les variables du module
    log.debug("  ... Strategie 1 echouee, scan de toutes les variables ...")
    for var_name in dir(module):
        if var_name.startswith("_"):
            continue
        obj = getattr(module, var_name, None)
        if obj is not None and _is_dash_or_flask(obj):
            log.info(f"  ✓  App Dash trouvée (scan) → variable '{var_name}' ({type(obj).__name__})")
            return module, obj

    # Strategie 3 : fallback par attributs .layout + .callback
    log.debug("  ... Strategie 2 echouee, recherche par attribut .layout ...")
    for var_name in dir(module):
        if var_name.startswith("_"):
            continue
        obj = getattr(module, var_name, None)
        if obj is None:
            continue
        if hasattr(obj, "layout") and hasattr(obj, "callback") and callable(getattr(obj, "callback", None)):
            log.info(f"  ✓  App Dash trouvée (fallback) → '{var_name}' ({type(obj).__name__})")
            return module, obj

    # Diagnostic : liste ce qui a ete trouve pour aider l'utilisateur
    found_vars = [
        f"  {v} ({type(getattr(module, v)).__name__})"
        for v in dir(module) if not v.startswith("_")
    ]
    diagnostic = "\n".join(found_vars[:30])
    raise ValueError(
        "Impossible de trouver l'objet Dash dans le fichier.\n\n"
        "Variables trouvees dans le module :\n"
        f"{diagnostic}\n\n"
        "Solutions :\n"
        "  1. Renommez votre objet Dash en `app`  ->  app = Dash(__name__)\n"
        "  2. Verifiez que le fichier ne leve pas d'exception a l'import\n"
        "     (lancez d'abord :  python votre_app.py  pour verifier)\n"
        "  3. Si votre app est dans une fonction, sortez-la au niveau module :\n"
        "     app = create_app()  <- appelez la factory au niveau racine\n"
    )
