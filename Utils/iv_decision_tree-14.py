...
class _ConformalMixin:
    ...
    def calibrate(self, X_cal, y_cal) -> None:
        ...
        if self._is_classifier():
            ...
            pred = proba_or_pred  # (n,)
            y_f  = y_cal.astype(np.float64)
            nc_scores = np.abs(y_f - pred)   # residuals as nonconformity scores
    ...
    def predict_set(self, X, alpha: float = 0.10) -> List[np.ndarray]:
        ...
        sets_idx = self._mcp_.predict_sets(leaf_ids, proba, alpha=alpha)
        return sets_idx
    ...
    def predict_interval(self, X, alpha: float = 0.10) -> np.ndarray:
        ...
        Examples
        --------
        >>> intervals = reg.predict_interval(X_test, alpha=0.10)
        >>> intervals[0]   # [lower_0, upper_0]
        """
        check_is_fitted(self)
        ...
        return np.column_stack([pred - q, pred + q])
    ...
    def conformal_diagnostics(self, X, y, alpha: float = 0.10) -> Dict[str, Any]:
        ...
        if self._is_classifier():
            ...
            y_int    = np.array([self._calib_cls2idx_.get(c, 0) for c in y_arr],
                                dtype=np.int32)
            return self._mcp_.coverage_diagnostics(leaf_ids, proba, y_int,
                                                   alpha=alpha)
        else:
            if not hasattr(self, '_mcp_reg_'):
                raise RuntimeError("Mondrian CP not fitted.")
            ...
            return {
                ...
                "avg_width":  float((intervals[:,1] - intervals[:,0]).mean()),
                "alpha":      alpha,
            }
...
class _MondrianConformalRegressor:
    ...
    At prediction: ŷ ± q_alpha  where q_alpha is the (1−alpha) quantile
    of calibration residuals in the same stratum.
    ...
    def __init__(self, min_leaf_size: int = 5):
        self.min_leaf_size   = min_leaf_size
        self._leaf_residuals: Dict[int, Optional[np.ndarray]] = {}
        self._global_res:     np.ndarray = np.array([])

    def fit(self, leaf_ids: np.ndarray,
            nc_scores: np.ndarray) -> "_MondrianConformalRegressor":
        """Store sorted absolute residuals per leaf."""
        self._global_res = np.sort(nc_scores)
        for lid in np.unique(leaf_ids):
            m = leaf_ids == lid
            if m.sum() < self.min_leaf_size:
                self._leaf_residuals[lid] = None
            else:
                self._leaf_residuals[lid] = np.sort(nc_scores[m])
        return self

    def predict_quantile(self, leaf_ids: np.ndarray,
                         alpha: float = 0.10) -> np.ndarray:
        """Return the (1−alpha) conformal quantile per test sample."""
        q = np.empty(len(leaf_ids))
        for i, lid in enumerate(leaf_ids):
            lid     = int(lid)
            _res = self._leaf_residuals.get(lid)
            cal_res = self._global_res if (_res is None) else _res
            n_cal   = len(cal_res)
            if n_cal == 0:
                q[i] = np.inf; continue
            # Conformal quantile: ceil((1-alpha)*(n+1))/n -th order statistic
            level = min(1.0, (1 - alpha) * (n_cal + 1) / n_cal)
            q[i]  = float(np.quantile(cal_res, level))
        return q
    ...
    def coverage_diagnostics(self, leaf_ids, y_true, y_pred,
        ...
        avg_width = float(2 * q.mean())
        return {"coverage": covered, "avg_width": avg_width, "alpha": alpha}
