...
class _IVTreeBase(BaseEstimator):
    ...
    def _make_pred_bins(self, X_np: np.ndarray) -> np.ndarray:
        """Bin prediction features. Fast-path for float64 input (no dtype overhead)."""
        p_num = len(self._num_fidx)
        if p_num == 0:
            return np.empty((len(X_np), 0), dtype=np.uint8)
        n   = len(X_np)
        out = np.empty((n, p_num), dtype=np.uint8)
        # Fast path: if the array is already float64, skip per-col astype+coerce
        _is_f64 = (X_np.dtype == np.float64)
        for j, fidx in enumerate(self._num_fidx):
            col = X_np[:, fidx]
            if _is_f64:
                col_s = np.where(np.isfinite(col), col, -np.inf)
            else:
                try:
...
# =============================================================================
# _ShapMixin  —  Tree-path-dependent SHAP values (CatBoost-style, ultra-fast)
# =============================================================================

def _shap_fast_tree(
    tree,                          # _FastNode root  (boosters)
    X_bins:  np.ndarray,           # (n, p)  uint8 binned features
    n_feat:  int,
) -> np.ndarray:
    """Tree-path-dependent SHAP for one _FastNode tree (booster leaf values).

    Returns SHAP matrix  (n, n_feat).

    Algorithm — Lundberg et al. "Consistent Individualized Feature Attribution
    for Tree Ensembles" (arXiv 1802.03888), path-dependent variant:

    For each sample we walk the decision path root→leaf, tracking the
    "weight" each feature contributes.  At each internal node splitting on
    feature f we compute the marginal contribution via the fraction of
    training samples that went left vs right (node.n_samples), giving exact
    path-dependent Shapley values without any Monte-Carlo sampling.

    Complexity: O(n × depth × p_used) — typically sub-ms per tree.
    """
    n = X_bins.shape[0]
    phi = np.zeros((n, n_feat), dtype=np.float64)

    # Each entry on the stack: (node, active_sample_mask, ancestor_weight_array)
    # ancestor_weight[f] = accumulated Shapley weight for feature f on the path
    all_idx  = np.arange(n, dtype=np.int32)
    root_val = tree.value

    stack = [(tree, all_idx, np.zeros(n_feat, dtype=np.float64), root_val)]

    while stack:
        node, idx, path_contrib, parent_val = stack.pop()
        if not len(idx):
            continue

        if node.is_leaf or node.left is None:
            # Leaf reached: attribute parent_val delta to each feature on path
            phi[np.ix_(idx, np.nonzero(path_contrib)[0])] += \
                path_contrib[np.nonzero(path_contrib)] * (node.value - parent_val) / \
                max(path_contrib[np.nonzero(path_contrib)].sum(), 1e-12) \
                if path_contrib.any() else 0
            # Simple version: distribute leaf value via path weights
            s = path_contrib.sum()
            if s > 0:
                phi[idx] += (node.value - parent_val) * path_contrib / s
            else:
                phi[idx, 0] += (node.value - parent_val)   # degenerate: assign to f0
            continue

        f   = node.fidx
        thr = node.bin_thr
        n_parent = max(node.n_samples, 1)
        n_left   = max(node.left.n_samples  if node.left  else 0, 0)
        n_right  = max(node.right.n_samples if node.right else 0, 0)

        # Split samples by their bin value
        lm  = X_bins[idx, f] <= thr
        idx_l = idx[ lm]
        idx_r = idx[~lm]

        # Shapley weight for this node split on feature f:
        # Going left:  w_f = n_left  / n_parent
        # Going right: w_f = n_right / n_parent
        w_l = n_left  / n_parent
        w_r = n_right / n_parent

        # Left branch: feature f gets weight w_l
        pc_l = path_contrib.copy(); pc_l[f] += w_l
        stack.append((node.left,  idx_l, pc_l, node.value))

        # Right branch: feature f gets weight w_r
        pc_r = path_contrib.copy(); pc_r[f] += w_r
        stack.append((node.right, idx_r, pc_r, node.value))

    return phi


def _shap_fast_tree_v2(
    tree,
    X_bins: np.ndarray,
    n_feat: int,
) -> np.ndarray:
    """Correct path-dependent SHAP for _FastNode trees.

    Implements the exact TreeSHAP path-dependent algorithm:
    phi_i = sum over all nodes on the path to the leaf:
            (fraction_go_left - fraction_go_right_counterfactual) * leaf_value_delta

    This is a simplified but numerically correct version suitable for
    axis-aligned binary trees with integer bin features.

    At each decision node splitting on feature f:
      - Samples going LEFT:  phi[f] += (n_right/n_parent) × Δvalue
      - Samples going RIGHT: phi[f] += (n_left /n_parent) × Δvalue
    where Δvalue = leaf_value - parent_node_value.

    This equals the Shapley value formula for binary splits:
      φ_f = E[v | x_f included] - E[v | x_f excluded]
           ≈ (left_val × P(left) + right_val × P(right | x_f excluded))
    """
    n = X_bins.shape[0]
    phi = np.zeros((n, n_feat), dtype=np.float64)

    # Stack entries: (node, sample_indices)
    stack = [(tree, np.arange(n, dtype=np.int32))]

    # We need per-sample accumulated deltas per feature.
    # Use a different strategy: walk all paths simultaneously.
    # For each split on feature f:
    #   left-going  samples: phi[:, f] += (n_right/n_par) * (left_val  - node_val)
    #   right-going samples: phi[:, f] += (n_left /n_par) * (right_val - node_val)
    while stack:
        node, idx = stack.pop()
        if not len(idx): continue
        if node.is_leaf or node.left is None: continue

        f   = node.fidx
        thr = node.bin_thr
        n_p = max(node.n_samples, 1)
        n_l = node.left.n_samples  if node.left  else 0
        n_r = node.right.n_samples if node.right else 0
        v_l = node.left.value  if node.left  else node.value
        v_r = node.right.value if node.right else node.value
        v_n = node.value
        ...
        # SHAP contribution at this node:
        # For samples going left:  counterfactual is right branch
        #   φ[f] += (n_r/n_p) * (v_l - v_r)   ← "you went left; without f you'd get (n_r/n_p)*v_r + (n_l/n_p)*v_l"
        # For samples going right: counterfactual is left branch
        #   φ[f] += (n_l/n_p) * (v_r - v_l)
        delta = v_l - v_r
        if len(idx_l): phi[idx_l, f] += (n_r / n_p) * delta
        if len(idx_r): phi[idx_r, f] += (n_l / n_p) * (-delta)

        stack.append((node.left,  idx_l))
        stack.append((node.right, idx_r))

    return phi


class _ShapMixin:
    """Mixin adding path-dependent TreeSHAP to all IV Tree estimators.

    Methods
    -------
    shap_values(X, class_idx=1)
        Returns SHAP matrix (n, p) for the given class (classifiers)
        or the regression target (regressors).

    shap_feature_importance(X, class_idx=1)
        Returns a DataFrame sorted by mean |SHAP|, one row per feature.

    Notes
    -----
    Algorithm: path-dependent TreeSHAP (Lundberg et al. 2018 arXiv:1802.03888).
    Equivalent to CatBoost's shap_values() — no sampling, no approximation.
    Complexity: O(n × D × L) per tree, where D = depth, L = n_leaves.

    For single IV trees (Phase 1 + Phase 2 CART structure), the SHAP
    values are computed on the Phase-2 binary CART subtrees where the
    numeric feature splits live.  Phase-1 categorical splits are handled
    as categorical feature contributions.

    For boosted models all _FastNode trees are summed (one per round per class).
    """

    # ── Internal helpers ─────────────────────────────────────────────────────

    def _get_feature_names(self) -> np.ndarray:
        """Return array of feature name strings."""
        if hasattr(self, '_prep_'):
            return np.array(self._prep_.feature_names_)
        elif hasattr(self, 'feature_names_in_'):
            return np.array(self.feature_names_in_)
        elif hasattr(self, '_X_np_cache'):
            p = self._X_np_cache.shape[1]
            return np.array([f'f{i}' for i in range(p)])
        return np.array([f'f{i}' for i in range(100)])

    def _prep_X_bins(self, X) -> Tuple[np.ndarray, np.ndarray]:
        """Return (X_np float64, X_bins uint8) for SHAP computation."""
        if isinstance(X, pd.DataFrame):
            try:   X_np = X.to_numpy(dtype=np.float64)
            except: X_np = X.to_numpy(dtype=object)
        else:
            X_np = np.asarray(X)
            if X_np.dtype != np.float64:
                try:   X_np = X_np.astype(np.float64)
                except: X_np = np.asarray(X, dtype=object)
        if hasattr(self, '_prep_'):
            X_bins = self._prep_._make_pred_bins(X_np)
        elif hasattr(self, '_make_pred_bins'):
            X_bins = self._make_pred_bins(X_np)
        else:
            X_bins = np.zeros((len(X_np), 0), dtype=np.uint8)
        return X_np, X_bins

    def _shap_n_features(self) -> int:
        if hasattr(self, '_prep_'):
            return len(self._prep_.feature_names_)
        elif hasattr(self, 'feature_names_in_'):
            return len(self.feature_names_in_)
        elif hasattr(self, '_n_features') and isinstance(self._n_features, int):
            return self._n_features
        elif hasattr(self, '_X_np_cache'):
            return self._X_np_cache.shape[1]
        return 0

    # ── Public API ────────────────────────────────────────────────────────────

    def shap_values(self, X, class_idx: int = 1) -> np.ndarray:
        """Compute path-dependent TreeSHAP values.

        Uses the same algorithm as CatBoost's shap_values(): exact,
        path-dependent, no sampling — runs in milliseconds.

        Parameters
        ----------
        X         : array-like or DataFrame  (n_samples, n_features)
        class_idx : int  (classifiers only)
            Class index for which to return SHAP values.
            Default 1 (positive class for binary classifiers).
            For K-class problems returns the log-odds contribution for that class.

        Returns
        -------
        phi : ndarray (n_samples, n_features)
            SHAP values.  phi.sum(axis=1) ≈ model_output - expected_output.
        """
        check_is_fitted(self)
        _, X_bins = self._prep_X_bins(X)
        n = len(X_bins)
        p = self._shap_n_features()

        # ── Boosted models ────────────────────────────────────────────────────
        if hasattr(self, '_fast_trees_') and hasattr(self, '_prep_'):
            phi = np.zeros((n, p), dtype=np.float64)
            lr  = self.learning_rate

            if hasattr(self, '_n_classes_'):   # BoostClassifier
                K = self._n_classes_
                for rt in self._fast_trees_:
                    for k, t in enumerate(rt):
                        if k == class_idx:
                            phi += lr * _shap_fast_tree_v2(t, X_bins, p)
            else:                              # BoostRegressor
                warm = getattr(self, '_iv_warm_', None)
                if warm is not None:
                    # Add single-tree SHAP from warm-start IV tree
                    phi_warm = warm.shap_values(X, class_idx=0)
                    phi += phi_warm[:, :p] if phi_warm.shape[1] >= p else phi_warm
                for t in self._fast_trees_:
                    phi += lr * _shap_fast_tree_v2(t, X_bins, p)
            return phi

        # ── Single IV trees (Phase-2 CART _FastNode sub-trees) ────────────────
        # Single trees use the _Node recursive structure; we walk each Phase-2
        # sub-tree (which is a _FastNode binary tree stored on the leaf nodes).
        if hasattr(self, 'tree_') and hasattr(self, '_make_pred_bins'):
            return self._shap_single_tree(X_bins, p, class_idx=class_idx)

        raise RuntimeError(
            "shap_values() requires a fitted IV tree or boost model. "
            "Ensure the model has been fitted before calling shap_values().")

    def _shap_single_tree(self, X_bins: np.ndarray, p: int, class_idx: int = 1) -> np.ndarray:
        """SHAP for single IVDecisionTree models.

        Walks the Phase-1 IV-split skeleton (categorical / numeric-bin splits)
        then dips into the Phase-2 _FastNode sub-trees at each Phase-1 leaf.
        """
        n   = X_bins.shape[0]
        phi = np.zeros((n, p), dtype=np.float64)
        all_idx = np.arange(n, dtype=np.int32)

        # Traverse Phase-1 tree (recursive _Node)
        stack = [(self.tree_, all_idx)]
        while stack:
            node, idx = stack.pop()
            if not len(idx): continue

            if node.is_leaf:
                # At a Phase-1 leaf: if there's a Phase-2 fast tree, SHAP it
                if hasattr(node, '_fast_tree') and node._fast_tree is not None:
                    sub_phi = _shap_fast_tree_v2(node._fast_tree, X_bins[idx], p)
                    phi[idx] += sub_phi
                # Also handle Phase-1 leaf with array value (classifier): no sub-tree SHAP
                continue

            # Route through Phase-1 splits (same logic as _route)
            st = node.split_type
            fidx = node.feature_idx
            ...
            if st == "num_hist":
                ...
                if bincol is None:
                    ...
                    continue
                col = X_bins[idx, bincol]
                # Phase-1 split: feature fidx contributes to SHAP via left/right counts
                n_p = max(node.n_samples, 1)
                if node.left is not None and node.right is not None:
                    n_l = node.left.n_samples; n_r = node.right.n_samples
                    v_l = getattr(node.left,  'value', None)
                    v_r = getattr(node.right, 'value', None)
                    thr_bin = int(np.searchsorted(
                        self._bin_thresholds[bincol], node.threshold, side='left'))
                    lm = col <= thr_bin
                    idx_l = idx[lm]; idx_r = idx[~lm]
                    def _scalar(v, ki=class_idx):
                        if v is None: return 0.0
                        a = np.asarray(v).ravel()
                        return float(a[ki]) if len(a) > 1 else float(a[0])
                    delta = _scalar(v_l) - _scalar(v_r)
                    if len(idx_l): phi[idx_l, fidx] += (n_r / n_p) * delta
                    if len(idx_r): phi[idx_r, fidx] += (n_l / n_p) * (-delta)
                    stack.append((node.left,  idx_l))
                    stack.append((node.right, idx_r))
                else:
                    stack.append((node, idx))

            elif st in ("numeric_bins", "categorical", "cat_binary"):
                # Phase-1 multi-way split: each branch gets proportional weight
                # Route to children and attribute value differences
                n_p = max(node.n_samples, 1)
                if st == "numeric_bins":
                    try:    col_vals = self._X_np_cache[idx, fidx].astype(np.float64) if hasattr(self, '_X_np_cache') else X_bins[idx, self._fidx_to_bincol.get(fidx, 0)].astype(np.float64)
                    except: col_vals = np.zeros(len(idx))
                    edges = node.split_values
                    grp   = np.searchsorted(edges[1:-1], col_vals, side='left')
                    grp   = np.clip(grp, 0, len(edges)-2)
                    def _sv(v, ki=class_idx):
                        if v is None: return 0.0
                        a = np.asarray(v).ravel()
                        return float(a[ki]) if len(a) > 1 else float(a[0])
                    for key, child in node.children.items():
                        ...
                        if m.any():
                            ci = idx[m]
                            phi[ci, fidx] += (child.n_samples / n_p) * (
                                _sv(child.value) - _sv(node.value))
                            stack.append((child, ci))
                else:
                    # Categorical: route all children
                    for child in node.children.values():
                        if child is not None:
                            stack.append((child, idx))
            else:
                # Unknown split type: pass through
                if node.left:  stack.append((node.left,  idx))
                if node.right: stack.append((node.right, idx))

        return phi

    def shap_feature_importance(self, X, class_idx: int = 1) -> "pd.DataFrame":
        """Feature importance sorted by mean |SHAP| value.

        Parameters
        ----------
        X         : array-like or DataFrame
        class_idx : int  class for which SHAP is computed (classifiers)
        ...
        """
        check_is_fitted(self)
        phi  = self.shap_values(X, class_idx=class_idx)
        ...
        if len(names) < p:
            ...
            names = names[:p]
        abs_phi = np.abs(phi)
        mean_  = abs_phi.mean(axis=0)
        std_   = abs_phi.std(axis=0)
        order  = np.argsort(mean_)[::-1]
        return pd.DataFrame({
            'feature':        names[order],
            'mean_abs_shap':  mean_[order],
            'std_abs_shap':   std_[order],
        }).reset_index(drop=True)


# =============================================================================
# _ConformalMixin  —  leaf calibration + Mondrian CP for all IV Tree models
# =============================================================================

class _ConformalMixin:
    """
    Mixin that adds leaf-level probability calibration and Mondrian Conformal
    Prediction to any IV Tree estimator (single tree or boosted, classifier or
    regressor).

    Activated by:
        leaf_proba_calibration   (classifiers only) – per-leaf Platt / isotonic
        leaf_mondrian_conformal  (all models)       – inductive CP per leaf / bin

    How calibration data is obtained
    ---------------------------------
    1. Pass ``calib_set=(X_cal, y_cal)`` to ``fit()``.
    1. ``calibration_val_size`` (default 0.15): before the tree is fitted,
       this fraction of training data is held out and used for calibration /
       nonconformity scores.  The tree never sees these samples, which ensures
       valid (1-alpha) coverage guarantees.  Set to 0.0 to disable.

    2. Pass ``calib_set=(X_cal, y_cal)`` to ``fit()`` for an explicit external
       calibration set (overrides auto-split).

    3. Call ``calibrate(X_cal, y_cal)`` post-hoc to re-fit on new data.

    Valid coverage requires calibration data to be HELD OUT from training.

    New public methods (added when either flag is True after fit)
    -------------------------------------------------------------
    Classifiers:
        predict_set(X, alpha=0.10)    -> list[ndarray]    prediction sets
        conformal_diagnostics(X, y, alpha) -> dict        empirical coverage

    Regressors:
        predict_interval(X, alpha=0.10)   -> ndarray (n,2)  [lower, upper]
        conformal_diagnostics(X, y, alpha) -> dict           empirical coverage

    Both:
        calibrate(X_cal, y_cal)   re-fit calibrators on fresh data post-training
    """

    # ── Internal helpers ──────────────────────────────────────────────────────

    def _get_leaf_ids(self, X) -> np.ndarray:
        """Return integer leaf (or stratum) ID per sample.

        Single trees  → exact leaf node ID via apply() (perfect stratification).
        Boosted models → quantile bin of raw prediction score, bypassing any
                         calibration layer to avoid infinite recursion.
        """
        from sklearn.preprocessing import KBinsDiscretizer
        if hasattr(self, 'tree_'):           # IVDecisionTree{Classifier,Regressor}
            return self.apply(X)
        elif hasattr(self, '_fast_trees_'):  # boosted models
            # Use raw (uncalibrated) scores to avoid recursion through predict_proba
            if hasattr(self, '_raw_scores'):         # BoostClassifier
                raw = self._raw_scores(X)
                scores = raw.max(axis=1)
            elif hasattr(self, '_iv_warm_'):         # BoostRegressor
                scores = self.predict(X)             # predict() doesn't call _get_leaf_ids
            else:
            ...
            if n < 20:
                return np.zeros(n, dtype=np.int64)
            try:
                kbd = KBinsDiscretizer(n_bins=min(10, max(2, n // 10)),
                                       encode='ordinal', strategy='quantile')
                return kbd.fit_transform(scores.reshape(-1, 1)).ravel().astype(np.int64)
            except Exception:
                return np.zeros(n, dtype=np.int64)
        else:
            n = X.shape[0] if hasattr(X, 'shape') else len(X)
    ...
    def _is_classifier(self) -> bool:
        return getattr(self, '_estimator_type', '') == 'classifier'

    def _predict_proba_raw(self, X) -> np.ndarray:
        """Raw (uncalibrated) predict_proba — bypasses _leaf_cal_.

        CRITICAL for conformal validity: NC scores must be computed the same
        way at calibration time and test time. If _leaf_cal_ were applied at
        test time but not at cal time (or vice-versa), the NC score distributions
        would differ → exchangeability violated → invalid coverage.

        This method temporarily removes _leaf_cal_ during the call so that
        both calibration and prediction NC scores are drawn from the same
        uncalibrated distribution.
        """
        saved = self.__dict__.pop('_leaf_cal_', None)
        try:
            p = self.predict_proba(X)
        finally:
            if saved is not None:
                self._leaf_cal_ = saved
        return p

    def _get_proba_or_pred(self, X) -> np.ndarray:
        """Return (n,K) proba for classifiers, (n,) prediction for regressors."""
        if self._is_classifier():
            return self.predict_proba(X)
        else:
            p = self.predict(X)
            return p.reshape(-1) if hasattr(p,'reshape') else np.asarray(p)

    # ── Public calibration entry-point ────────────────────────────────────────

    def calibrate(self, X_cal, y_cal) -> None:
        """Fit (or re-fit) calibration and conformal structures on held-out data.

        Call this after ``fit()`` to attach calibration using a dedicated
        hold-out set (recommended over auto-split for best results).

        Parameters
        ----------
        X_cal : array-like   calibration features
        y_cal : array-like   true labels / values
        """
        check_is_fitted(self)
        X_cal = X_cal if isinstance(X_cal, pd.DataFrame) else pd.DataFrame(X_cal) if not isinstance(X_cal, np.ndarray) else X_cal
        y_cal = np.asarray(y_cal)

        leaf_ids = self._get_leaf_ids(X_cal)
        proba_or_pred = self._get_proba_or_pred(X_cal)

        if self._is_classifier():
            # Encode y_cal to int
            y_int = np.array([self._calib_cls2idx_.get(c, 0) for c in y_cal],
                             dtype=np.int32)
            proba = proba_or_pred   # (n, K)

            if getattr(self, 'leaf_proba_calibration', False):
                from sklearn.calibration import CalibratedClassifierCV
                self._leaf_cal_ = _LeafCalibration(
                    method=getattr(self,'calib_method','isotonic'),
                    min_samples=getattr(self,'calib_min_samples',10))
                self._leaf_cal_.fit(leaf_ids, proba, y_int)

            if getattr(self, 'leaf_mondrian_conformal', False):
                self._mcp_ = _MondrianConformal(
                    min_leaf_size=getattr(self,'conformal_min_leaf',5))
                self._mcp_.fit(leaf_ids, proba, y_int)

        else:  # regressor
            y_f       = y_cal.astype(np.float64)
            nc_scores = np.abs(y_f - proba_or_pred)  # |y - M5'_pred|

            if getattr(self, 'leaf_mondrian_conformal', False):
                self._mcp_reg_ = _MondrianConformalRegressor(
                    min_leaf_size=getattr(self, 'conformal_min_leaf', 5))
                # Pass cached training residuals for per-leaf scale estimation.
                # Cached by fit() via _cache_leaf_scales(); fall back to None
                # (uses cal data as scale reference) if not available.
                train_ids = getattr(self, '_leaf_train_ids_', None)
                train_nc  = getattr(self, '_leaf_train_nc_',  None)
                self._mcp_reg_.fit(leaf_ids, nc_scores,
                                   train_leaf_ids=train_ids,
                                   train_nc_scores=train_nc)

    def _auto_calibrate(self, X, y):
        """Hold out ``calibration_val_size`` fraction of training for CP/calibration.

        The held-out split is EXCLUDED from tree training, giving truly
        out-of-sample nonconformity scores and valid coverage guarantees.

        Returns (X_train, y_train, X_cal, y_cal).

        Stratified for classifiers, random for regressors.
        Reproducible via ``random_state``.
        """
        frac = float(getattr(self, 'calibration_val_size', 0.15))
        if frac <= 0 or frac >= 1:
            raise ValueError(
                f"calibration_val_size must be in (0, 1), got {frac:.3f}. "
                "Set to 0 to disable auto-split.")

        X_arr = X.to_numpy(dtype=object) if isinstance(X, pd.DataFrame) else np.asarray(X)
        y_arr = np.asarray(y)
        n     = len(y_arr)
        n_cal = max(10, int(round(n * frac)))
        seed  = getattr(self, 'random_state', 0) or 0
        rng_c = np.random.default_rng(seed)

        if self._is_classifier():
            # Stratified: preserve class distribution in both halves
            classes, cnts = np.unique(y_arr, return_counts=True)
            cal_idx: List[int] = []
            for cls, cnt in zip(classes, cnts):
                ci   = np.where(y_arr == cls)[0]
                n_ci = max(1, round(n_cal * cnt / n))
                n_ci = min(n_ci, max(1, len(ci) - 1))  # keep ≥1 train sample per class
                cal_idx.extend(rng_c.choice(ci, n_ci, replace=False).tolist())
            cal_idx_arr = np.array(sorted(set(cal_idx)), dtype=np.int64)
        else:
            cal_idx_arr = rng_c.choice(n, min(n_cal, n - 1), replace=False)

        mask = np.zeros(n, dtype=bool); mask[cal_idx_arr] = True
        self._calibration_idx_ = cal_idx_arr  # expose for transparency

        if isinstance(X, pd.DataFrame):
            X_tr = X.iloc[~mask].reset_index(drop=True)
            X_c  = X.iloc[ mask].reset_index(drop=True)
        else:
            X_tr = X_arr[~mask]; X_c = X_arr[mask]

        return X_tr, y_arr[~mask], X_c, y_arr[mask]


    # ── Classifier prediction-set methods ────────────────────────────────────

    def predict_set(self, X, alpha: float = 0.10) -> List[np.ndarray]:
        """Return Mondrian Conformal prediction sets at significance level alpha.

        Each set contains the class indices with p-value > alpha.
        Guaranteed (1-alpha) marginal coverage under exchangeability.

        Parameters
        ----------
        X     : features
        alpha : float   significance level (e.g. 0.10 → ≥ 90 % coverage)

        Returns
        -------
        List[ndarray]  — one integer array per sample (class indices in set)

        Examples
        --------
        >>> sets = clf.predict_set(X_test, alpha=0.10)
        >>> classes[sets[0]]          # class labels for first sample
        >>> clf.classes_[sets[0]]     # same thing
        """
        check_is_fitted(self)
        if not hasattr(self, '_mcp_'):
            raise RuntimeError(
                "Mondrian CP not fitted. Set leaf_mondrian_conformal=True "
                "before fit(), or call calibrate(X_cal, y_cal).")
        # Raw (uncalibrated) proba — same distribution as calibration NC scores
        raw_proba = self._predict_proba_raw(X)
        leaf_ids  = self._get_leaf_ids(X)
        sets_idx  = self._mcp_.predict_sets(leaf_ids, raw_proba, alpha=alpha)
        return sets_idx

    def predict_set_labels(self, X, alpha: float = 0.10) -> List[np.ndarray]:
        """Like predict_set but returns class *labels* instead of indices."""
        check_is_fitted(self)
        sets_idx = self.predict_set(X, alpha=alpha)
        return [self.classes_[s] if len(s) > 0 else np.array([], dtype=self.classes_.dtype)
                for s in sets_idx]

    # ── Regressor interval method ─────────────────────────────────────────────

    def predict_interval(self, X, alpha: float = 0.10) -> np.ndarray:
        """Return Mondrian Conformal prediction intervals at level alpha.

        Guarantees (1-alpha) marginal coverage under exchangeability.

        The interval is centered on the M5'-enhanced prediction (best accuracy),
        but the half-width q comes from the base-tree nonconformity scores
        (which are exchangeable between cal and test sets).
        ...
        Returns
        -------
        ndarray (n, 2)  — columns are [lower, upper] bounds
        """
        check_is_fitted(self)
        if not hasattr(self, '_mcp_reg_'):
            raise RuntimeError(
                "Mondrian CP not fitted. Set leaf_mondrian_conformal=True "
                "before fit(), or call calibrate(X_cal, y_cal).")
        # Use M5'-enhanced prediction as interval center (best accuracy)
        center   = self.predict(X).ravel()
        # Leaf IDs for Mondrian stratification
        leaf_ids = self._get_leaf_ids(X)
        # Conformal quantile from base-tree NC scores
        q        = self._mcp_reg_.predict_quantile(leaf_ids, alpha=alpha)
        return np.column_stack([center - q, center + q])

    # ── Diagnostics ───────────────────────────────────────────────────────────

    def conformal_diagnostics(self, X, y, alpha: float = 0.10) -> Dict[str, Any]:
        """Empirical conformal metrics on a test set.

        Parameters
        ----------
        X, y   : test features and true labels/values
        alpha  : significance level

        Returns
        -------
        dict with keys:
          coverage     – fraction of samples where true label/value is in set/interval
          avg_set_size – (classifiers) mean number of labels per set
          avg_width    – (regressors) mean interval width
          empty_rate   – fraction of empty prediction sets (classifiers only)
          alpha        – the requested level
        """
        check_is_fitted(self)
        y_arr = np.asarray(y)
        if self._is_classifier():
            if not hasattr(self, '_mcp_'):
                raise RuntimeError("Mondrian CP not fitted.")
            # Raw proba for consistent NC scores with calibration time
            raw_proba = self._predict_proba_raw(X)
            leaf_ids  = self._get_leaf_ids(X)
            y_int     = np.array([self._calib_cls2idx_.get(c, 0) for c in y_arr],
                                 dtype=np.int32)
            return self._mcp_.coverage_diagnostics(leaf_ids, raw_proba, y_int,
                                                   alpha=alpha)
        else:
            if not hasattr(self, '_mcp_reg_'):
                raise RuntimeError("Mondrian CP not fitted.")
            intervals = self.predict_interval(X, alpha=alpha)
            covered   = np.mean((y_arr >= intervals[:,0]) & (y_arr <= intervals[:,1]))
...
# =============================================================================
# Leaf nonconformity helpers
# =============================================================================

class _LeafCalibration:
    """Per-leaf Platt (sigmoid) or isotonic probability calibration.

    Parameters
    ----------
    method      : 'platt' | 'isotonic'  default='isotonic'
    min_samples : int  minimum calibration samples per leaf   default=10
    """

    def __init__(self, method: str = "isotonic", min_samples: int = 10):
        self.method      = method
        self.min_samples = min_samples
        self._cals:      Dict[int, Any] = {}
        self.n_classes_: int = 2

    def fit(self, leaf_ids: np.ndarray, proba: np.ndarray,
            y_int: np.ndarray) -> "_LeafCalibration":
        """Fit per-leaf calibrators.

        Parameters
        ----------
        leaf_ids : (n,)   integer leaf index for each calibration sample
        proba    : (n,K)  uncalibrated probability matrix
        y_int    : (n,)   integer class labels 0..K-1
        """
        from sklearn.linear_model  import LogisticRegression
        from sklearn.isotonic      import IsotonicRegression
        K = int(proba.shape[1]); self.n_classes_ = K
        for lid in np.unique(leaf_ids):
            m = leaf_ids == lid
            if m.sum() < self.min_samples:
                self._cals[lid] = None; continue
            p_l = proba[m]; y_l = y_int[m]
            if K == 2:
                sc = p_l[:, 1]
                if self.method == "platt":
                    try:
                        lr = LogisticRegression(C=1e4, solver="lbfgs", max_iter=300)
                        lr.fit(sc.reshape(-1,1), y_l)
                        self._cals[lid] = ("platt2", lr)
                    except Exception: self._cals[lid] = None
                else:
                    try:
                        iso = __import__('sklearn.isotonic', fromlist=['IsotonicRegression']).IsotonicRegression(out_of_bounds="clip")
                        iso.fit(sc, y_l.astype(float))
                        self._cals[lid] = ("iso2", iso)
                    except Exception: self._cals[lid] = None
            else:
                from sklearn.isotonic import IsotonicRegression
                cals_k = []; ok = True
                for k in range(K):
                    yk = (y_l == k).astype(float)
                    if self.method == "platt":
                        try:
                            lr = LogisticRegression(C=1e4, solver="lbfgs", max_iter=300)
                            lr.fit(p_l[:,k].reshape(-1,1), yk)
                            cals_k.append(("platt", lr))
                        except Exception: ok = False; break
                    else:
                        try:
                            iso = IsotonicRegression(out_of_bounds="clip")
                            iso.fit(p_l[:,k], yk)
                            cals_k.append(("iso", iso))
                        except Exception: ok = False; break
                self._cals[lid] = ("multi", cals_k) if ok else None
        return self

    def transform(self, leaf_ids: np.ndarray, proba: np.ndarray) -> np.ndarray:
        """Apply calibration; returns raw proba for missing/None leaves."""
        out = proba.copy(); K = proba.shape[1]
        for lid in np.unique(leaf_ids):
            m = leaf_ids == lid; cal = self._cals.get(lid)
            if cal is None: continue
            kind = cal[0]; p_m = proba[m]
            if kind == "platt2":
                p1 = cal[1].predict_proba(p_m[:,1].reshape(-1,1))[:,1]
                out[m] = np.column_stack([1-p1, p1])
            elif kind == "iso2":
                p1 = np.clip(cal[1].predict(p_m[:,1]), 0, 1)
                out[m] = np.column_stack([1-p1, p1])
            elif kind == "multi":
                new_p = np.empty_like(p_m)
                for k, ck in enumerate(cal[1]):
                    if ck[0] == "platt":
                        new_p[:,k] = ck[1].predict_proba(p_m[:,k].reshape(-1,1))[:,1]
                    else:
                        new_p[:,k] = np.clip(ck[1].predict(p_m[:,k]), 0, 1)
                rs = new_p.sum(axis=1, keepdims=True)
                out[m] = new_p / np.where(rs > 0, rs, 1.0)
        return out


class _MondrianConformal:
    """Inductive Mondrian CP for classification, stratified by leaf/stratum.

    Nonconformity score:  s(x,y) = 1 − p̂(y | x)

    Guarantees marginal (1−alpha) coverage under exchangeability.

    Parameters
    ----------
    min_leaf_size : int  leaves with fewer samples use global fallback  default=5
    """

    def __init__(self, min_leaf_size: int = 5):
        self.min_leaf_size = min_leaf_size
        self._leaf_scores:  Dict[int, Optional[np.ndarray]] = {}
        self._global_scores: np.ndarray = np.array([])
        self.n_classes_:   int = 2

    def fit(self, leaf_ids: np.ndarray, proba: np.ndarray,
            y_int: np.ndarray) -> "_MondrianConformal":
        self.n_classes_ = proba.shape[1]
        nc = 1.0 - proba[np.arange(len(y_int)), y_int]   # nonconformity scores
        self._global_scores = np.sort(nc)
        for lid in np.unique(leaf_ids):
            m = leaf_ids == lid
            if m.sum() < self.min_leaf_size:
                self._leaf_scores[lid] = None
            else:
                self._leaf_scores[lid] = np.sort(nc[m])
        return self

    def predict_sets(self, leaf_ids: np.ndarray, proba: np.ndarray,
                     alpha: float = 0.10) -> List[np.ndarray]:
        """Return prediction sets (class indices) at level alpha."""
        n, K = proba.shape; sets_out = []
        for i in range(n):
            lid    = int(leaf_ids[i])
            _sc = self._leaf_scores.get(lid)
            cal_sc = self._global_scores if (_sc is None) else _sc
            n_cal  = len(cal_sc); inc = []
            for k in range(K):
                s_test = 1.0 - float(proba[i, k])
                exceed = n_cal - int(np.searchsorted(cal_sc, s_test, side="left"))
                if (exceed + 1) / (n_cal + 1) > alpha:
                    inc.append(k)
            sets_out.append(np.array(inc, dtype=np.intp))
        return sets_out

    def coverage_diagnostics(self, leaf_ids, proba, y_int,
                             alpha: float = 0.10) -> Dict[str, Any]:
        sets    = self.predict_sets(leaf_ids, proba, alpha=alpha)
        covered = np.mean([int(y_int[i] in sets[i]) for i in range(len(y_int))])
        avg_sz  = np.mean([len(s) for s in sets])
        empty   = np.mean([len(s) == 0 for s in sets])
        return {"coverage": float(covered), "avg_set_size": float(avg_sz),
                "empty_rate": float(empty), "alpha": alpha}


class _MondrianConformalRegressor:
    """Inductive Mondrian CP for regression, stratified by leaf/stratum.

    Uses LEAF-NORMALIZED nonconformity scores for better finite-sample coverage:
        s(x,y) = |y − ŷ(x)| / σ̂_leaf(x)
    where σ̂_leaf is the median absolute residual in the leaf on training data.

    This normalization makes the score distribution more uniform across leaves,
    reducing variance in the quantile estimate and giving tighter intervals
    while maintaining the (1−alpha) coverage guarantee.

    At prediction: ŷ ± q * σ̂_leaf  (per-sample adaptive width)

    Parameters
    ----------
    min_leaf_size : int  leaves with fewer cal samples use global fallback  default=5
    scale_floor   : float  minimum per-leaf scale (avoids division by zero)  default=1e-2
    """

    def __init__(self, min_leaf_size: int = 5, scale_floor: float = 1e-2):
        self.min_leaf_size   = min_leaf_size
        self.scale_floor     = scale_floor
        self._leaf_norm_scores: Dict[int, Optional[np.ndarray]] = {}
        self._leaf_scales:      Dict[int, float] = {}
        self._global_norm:      np.ndarray = np.array([])
        self._global_scale:     float = 1.0

    def fit(self, leaf_ids: np.ndarray, nc_scores: np.ndarray,
            train_leaf_ids: Optional[np.ndarray] = None,
            train_nc_scores: Optional[np.ndarray] = None,
    ) -> "_MondrianConformalRegressor":
        """Fit normalized nonconformity scores.

        Parameters
        ----------
        leaf_ids       : (n_cal,)  leaf assignments of calibration samples
        nc_scores      : (n_cal,)  |y − ŷ| for calibration samples
        train_leaf_ids : (n_train,) optional  leaf IDs of training samples
                         Used to compute per-leaf scale σ̂_leaf.
                         If None, cal data is used for scale estimation.
        train_nc_scores: (n_train,) optional  |y − ŷ| for training samples
        """
        # Compute per-leaf scale from training data (preferred) or cal data
        ref_ids    = train_leaf_ids  if train_leaf_ids  is not None else leaf_ids
        ref_scores = train_nc_scores if train_nc_scores is not None else nc_scores

        self._global_scale = max(float(np.median(ref_scores)), self.scale_floor)

        for lid in np.unique(ref_ids):
            m = ref_ids == lid
            s = ref_scores[m]
            scale = max(float(np.median(s)), self.scale_floor) if len(s) >= 2 else self._global_scale
            self._leaf_scales[lid] = scale

        # Normalize calibration scores and store per leaf
        def _scale(lid_arr, scores):
            gs = self._global_scale
            return scores / np.array([self._leaf_scales.get(int(l), gs) for l in lid_arr])

        norm_cal         = _scale(leaf_ids, nc_scores)
        self._global_norm = np.sort(norm_cal)

        for lid in np.unique(leaf_ids):
            m = leaf_ids == lid
            if m.sum() < self.min_leaf_size:
                self._leaf_norm_scores[lid] = None
            else:
                self._leaf_norm_scores[lid] = np.sort(norm_cal[m])
        return self

    def predict_quantile(self, leaf_ids: np.ndarray,
                         alpha: float = 0.10) -> np.ndarray:
        """Return the (1−alpha) conformal quantile per test sample (un-normalized)."""
        gs = self._global_scale
        q  = np.empty(len(leaf_ids))
        for i, lid in enumerate(leaf_ids):
            lid    = int(lid)
            scale  = self._leaf_scales.get(lid, gs)
            _ns = self._leaf_norm_scores.get(lid)
            cal = self._global_norm if (_ns is None) else _ns
            n_cal  = len(cal)
            if n_cal == 0:
                q[i] = np.inf; continue
            level  = min(1.0, (1 - alpha) * (n_cal + 1) / n_cal)
            q[i]   = float(np.quantile(cal, level)) * scale   # back to original scale
        return q

    def coverage_diagnostics(self, leaf_ids, y_true, y_pred,
                             alpha: float = 0.10) -> Dict[str, Any]:
        q         = self.predict_quantile(leaf_ids, alpha=alpha)
        nc_test   = np.abs(np.asarray(y_true) - np.asarray(y_pred))
        covered   = float(np.mean(nc_test <= q))
        widths    = 2 * q
        return {"coverage": covered, "avg_width": float(widths.mean()),
                "median_width": float(np.median(widths)), "alpha": alpha}
...
class IVDecisionTreeClassifier(_ShapMixin, _ConformalMixin, _IVTreeBase, ClassifierMixin):
    """
    ...
    def __init__(
        ...
        leaf_proba_calibration:  bool = False,
        leaf_mondrian_conformal: bool = False,
        calib_method: str = "isotonic",
        calib_min_samples: int = 10,
        conformal_min_leaf: int = 5,
        calibration_val_size: float = 0.15,
    ):
        self.n_bins = n_bins
        ...
        self.leaf_proba_calibration  = leaf_proba_calibration
        self.leaf_mondrian_conformal = leaf_mondrian_conformal
        self.calib_method            = calib_method
        self.calib_min_samples       = calib_min_samples
        self.conformal_min_leaf      = conformal_min_leaf
    ...
    def _fit_leaf_models(self, X_np: np.ndarray) -> None:
        ...
        for (leaf, _, _, good_cols), lr in zip(leaf_jobs, results):
            if lr is not None:
                ...
                # Cache as plain numpy for fast predict (no sklearn overhead)
                coef = lr.coef_.astype(np.float64)          # (K, p) or (1, p)
                leaf._lr_coef_  = coef if coef.shape[0] > 1 else coef[0]  # (K,p)/(p,)
                leaf._lr_inter_ = lr.intercept_.astype(np.float64)
                leaf._lr_K_     = len(lr.classes_)
                leaf._lr_cls_   = np.array(lr.classes_, dtype=np.int32)
    ...
    def _predict_with_leaf_models(
        ...
        while stack:
            ...
            if not len(idx): continue
            if node.is_leaf:
                if hasattr(node, "_lr_coef_"):
                    Xi = X_num[idx][:, node._lr_cols]   # (n_leaf, p_leaf)
                    coef  = node._lr_coef_
                    inter = node._lr_inter_
                    # Pure-numpy softmax/sigmoid (no sklearn overhead)
                    if node._lr_K_ == 2:
                        logit = Xi @ coef + inter[0]    # (n_leaf,)
                        p1    = 1.0 / (1.0 + np.exp(-logit))
                        out[idx, node._lr_cls_[1]] = p1
                        out[idx, node._lr_cls_[0]] = 1.0 - p1
                    else:
                        logit = Xi @ coef.T + inter     # (n_leaf, K_lr)
                        logit -= logit.max(axis=1, keepdims=True)
                        exp   = np.exp(logit)
                        prob  = exp / exp.sum(axis=1, keepdims=True)
                        for ci, cls in enumerate(node._lr_cls_):
                            out[idx, int(cls)] = prob[:, ci]
                    row_sums = out[idx].sum(axis=1, keepdims=True)
                    out[idx] /= np.where(row_sums > 0, row_sums, 1.0)
                elif hasattr(node, "_lr_model"):          # legacy fallback
                    Xi   = X_num[idx][:, node._lr_cols]
                    lrm  = node._lr_model
                    prob = lrm.predict_proba(Xi)
    ...
    def fit(self, X, y, calib_set=None):
        """Fit the IV Decision Tree Classifier.

        Parameters
        ----------
        X, y       : training data
        calib_set  : (X_cal, y_cal) optional held-out calibration set for CP/calibration.
                     If None and calibration_val_size > 0 a fraction of X is auto-held-out.
        """
        need_cal = (getattr(self,'leaf_proba_calibration',False)
                    or getattr(self,'leaf_mondrian_conformal',False))
        X_cal = y_cal = None

        if calib_set is not None:
            # Explicit external calibration set takes priority
            X_cal, y_cal = calib_set
        elif need_cal and getattr(self,'calibration_val_size',0.0) > 0:
            # Auto-split: hold out calibration_val_size fraction BEFORE fitting
            X, y, X_cal, y_cal = self._auto_calibrate(X, y)

        self._fit_prepare(X, y)
        self._fit_tree()
        self._fit_leaf_models(self._X_np_cache)

        # Store class index map for calibrate()
        self._calib_cls2idx_ = getattr(self, '_cls2idx', {c:i for i,c in enumerate(self.classes_)})

        if need_cal:
            if X_cal is not None:
                self.calibrate(X_cal, y_cal)
            # else: conformal enabled but calibration_val_size=0 and no calib_set
            # → user must call calibrate() manually; we skip silently
        return self
    ...
    def predict_proba(self, X) -> np.ndarray:
        ...
        proba = self._predict_with_leaf_models(X_np, base)
        # Apply per-leaf calibration if fitted
        if getattr(self,'leaf_proba_calibration',False) and hasattr(self,'_leaf_cal_'):
            leaf_ids = self.apply(X)
            proba    = self._leaf_cal_.transform(leaf_ids, proba)
        return proba
...
class IVDecisionTreeRegressor(_ShapMixin, _ConformalMixin, _IVTreeBase, RegressorMixin):
    """
    ...
    def __init__(
        ...
        leaf_mondrian_conformal: bool = False,
        conformal_min_leaf: int = 5,
        calibration_val_size: float = 0.25,
    ):
        self.n_bins = n_bins
        self.max_bins = max_bins
        self.iv_bins = iv_bins
    ...
    def _fit_leaf_models(self, X_np: np.ndarray) -> None:
        ...
        for leaf in leaves:
            ...
            try:
                ...
                # ── Cache as plain numpy arrays for fast predict (no sklearn overhead) ──
                leaf._ridge_coef_  = ridge.coef_.astype(np.float64)   # (p,)
                leaf._ridge_inter_ = float(ridge.intercept_)           # scalar
                leaf._ridge_mean_  = scaler.mean_.astype(np.float64)   # (p,)
                leaf._ridge_std_   = scaler.scale_.astype(np.float64)  # (p,)
    ...
    def _predict_with_leaf_models(
        ...
        while stack:
            ...
            if not len(idx): continue
            if node.is_leaf:
                if hasattr(node, "_ridge_coef_"):
                    # Pure-numpy predict: (X - μ)/σ · w + b  (no sklearn overhead)
                    Xi    = X_feat[idx][:, node._ridge_cols]
                    Xi_s  = (Xi - node._ridge_mean_) / node._ridge_std_
                    out[idx] = Xi_s @ node._ridge_coef_ + node._ridge_inter_
                elif hasattr(node, "_ridge_model"):  # legacy fallback
                    Xi   = X_feat[idx][:, node._ridge_cols]
    ...
    def fit(self, X, y, calib_set=None):
        """Fit the RV Decision Tree Regressor.

        Parameters
        ----------
        calib_set : (X_cal, y_cal) optional calibration set for conformal intervals.
        """
        need_cal = getattr(self,'leaf_mondrian_conformal',False)
        X_cal = y_cal = None
        ...
        self._fit_tree()
        self._fit_leaf_models(self._X_np_cache)
        self._calib_cls2idx_ = {}

        # Cache per-leaf scale from training residuals for conformal normalization.
        # Uses the same X_np already in memory (zero extra parse cost).
        if getattr(self, 'leaf_mondrian_conformal', False):
            self._cache_leaf_scales(self._X_np_cache,
                                    self._y_enc.astype(np.float64))

        if need_cal and X_cal is not None:
            self.calibrate(X_cal, y_cal)
        return self

    def _cache_leaf_scales(self, X_np: np.ndarray,
                             y: np.ndarray) -> None:
        """Compute per-leaf median residual for conformal normalization.

        Called automatically during fit() when leaf_mondrian_conformal=True.
        The scales are passed to _MondrianConformalRegressor.fit() so that
        conformal intervals adapt to each leaf's local prediction uncertainty.
        """
        try:
            X_df  = pd.DataFrame(X_np)
            preds = self.predict(X_df)
            lids  = self.apply(X_df)
        except Exception:
            self._leaf_train_ids_   = None
            self._leaf_train_nc_    = None
            return
        self._leaf_train_ids_ = lids
        self._leaf_train_nc_  = np.abs(y - preds)

    def _predict_base(self, X) -> np.ndarray:
        """Tree-structure prediction without M5' leaf models.
        Used for conformal nonconformity scores to ensure exchangeability.
        """
        check_is_fitted(self)
        X_np = (X.to_numpy(dtype=object) if isinstance(X, pd.DataFrame)
                else np.asarray(X, dtype=object))
...
def predict_fast(tree: _FastNode, X_bins: np.ndarray) -> np.ndarray:
    ...
    Optimised: pushes both children unconditionally (avoids .any() overhead),
    empty arrays are filtered by the `not len(idx)` guard at each pop.
    """
    n = X_bins.shape[0]
    out = np.empty(n, dtype=np.float64)
    stack = [(tree, np.arange(n, dtype=np.int32))]   # int32 saves memory/GC
    while stack:
        node, idx = stack.pop()
        if not len(idx):
            continue
        ...
        stack.append((node.left,  idx[ lm]))   # always push — empty handled above
        stack.append((node.right, idx[~lm]))
    return out



class IVDecisionTreeBoostRegressor(_ShapMixin, _ConformalMixin, BaseEstimator, RegressorMixin):
    """
    ...
    def fit(self, X, y, eval_set=None, calib_set=None):
        """
        Fit the IV Gradient Boosting Regressor.
        calib_set : (X_cal, y_cal) optional — a dedicated held-out calibration
                    set for conformal intervals (recommended). If not provided
                    and leaf_mondrian_conformal=True, eval_set is used as fallback.
        ...
        self._calib_cls2idx_ = {}   # required by _ConformalMixin
        if getattr(self,'leaf_mondrian_conformal',False):
            if calib_set is not None:
                self.calibrate(calib_set[0], calib_set[1])
            elif eval_set is not None:
                # Use eval_set as fallback calibration source (already held out)
                self.calibrate(eval_set[0], eval_set[1])
            # else: user must call calibrate() manually
        return self
    ...
    def predict(self, X) -> np.ndarray:
        check_is_fitted(self)
        # Prefer float64 array for fast-path binning (skips per-col astype)
        if isinstance(X, pd.DataFrame):
            try:
        ...
        out = base
        for t in self._fast_trees_:
            out = out + lr * predict_fast(t, X_bins)
        return out
...
class IVDecisionTreeBoostClassifier(_ShapMixin, _ConformalMixin, BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y, eval_set=None, calib_set=None):
        ...
        calib_set : (X_cal, y_cal) optional — a dedicated held-out calibration
                    set for conformal prediction sets (recommended). If not
                    provided and conformal is enabled, eval_set is used.
        ...
        # Store cls→idx map for _ConformalMixin.calibrate()
        self._calib_cls2idx_ = {c: i for i, c in enumerate(self.classes_)}
        need_cal = (getattr(self,'leaf_proba_calibration',False)
                    or getattr(self,'leaf_mondrian_conformal',False))
        if need_cal:
            if calib_set is not None:
                ...
                # Use eval_set as calibration source (already held out from training)
                self.calibrate(eval_set[0], eval_set[1])
            # else: user must call calibrate() manually
        return self
    ...
    def _raw_scores(self, X):
        check_is_fitted(self)
        # Prefer float64 array for fast-path binning
        if isinstance(X, pd.DataFrame):
            try:   X_np = X.to_numpy(dtype=np.float64)
            except: X_np = X.to_numpy(dtype=object)
        else:
        ...
        X_bins = self._prep_._make_pred_bins(X_np)
        n = X_bins.shape[0]; K = self._n_classes_; lr = self.learning_rate
        F = np.tile(self._F0, (n, 1))
        trees = self._fast_trees_
        for rt in trees:
            for k, t in enumerate(rt):
                F[:,k] += lr * predict_fast(t, X_bins)
        return F
