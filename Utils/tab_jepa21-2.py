...
def _train_one_epoch(
    ...
    for batch in loader:
        ...
        optimizer.zero_grad()
        loss.backward()
        all_p = list(model.encoder.parameters()) + (
            list(supervised_head.parameters()) if supervised_head else []
        )
...
def _run_training(
    ...
    if supervised_head is not None:
        param_groups = [
            {"params": list(supervised_head.parameters()), "lr": lr_head or base_lr},
        ]
        if model.encoder.parameters:
            param_groups.append({
                "params": [p for p in model.encoder.parameters() if p.requires_grad],
                "lr": lr_encoder or base_lr,
            })
    else:
        param_groups = [
...
class TabJEPA21Trainer:
    ...
    def __init__(
        ...
        self._ctor_kwargs = dict(
            ...
            n_predictor_layers=n_predictor_layers, d_pred=d_pred,
...
class TabJEPA21Supervised:
    ...
    def fit(
        ...
        if verbose:
            print(
                f"[val_size={self.val_size}] split → train={len(tr_idx)} | val={len(n_val if False else len(va_idx))}\n"
        ...
        _run_training(
            ...
            lr_head=self.lr_head, lr_encoder=self.lr_encoder,
        )
